...
namespace wp {
	...
	obj_function::obj_function(cv::Mat dist_map, float lambda, float initial_x1, float initial_y1, float initial_x2, float initial_y2) {
	...
	double obj_function::operator() (const column_vector& arg) const {
		...
		if (y2 >= 227) y2 = 226;
		*/
		return distance(dist_map, lambda, initial_x1, initial_y1, initial_x2, initial_y2, arg(0, 0), arg(1, 0), arg(2, 0), arg(3, 0));
	}



	std::vector<float> parameterEstimation(boost::shared_ptr<Regression> regression, const cv::Mat& tile, bool refine, float lambda, int maxIters) {
		// prediction
		...
		if (refine) {
			...
			try {
				find_min_bobyqa(obj_function(dist, lambda, params[0], params[1], params[2], params[3]),
					starting_point,
					10,
					lower_bound,
					upper_bound,
	...
	float distance(const cv::Mat& dist_map, float lambda, float initial_x1, float initial_y1, float initial_x2, float initial_y2, float x1, float y1, float x2, float y2) {
		float diff = 0;
		int cnt = 0;
		...
		return diff / cnt + (abs(x1 - initial_x1) + abs(y1 - initial_y1) + abs(x2 - initial_x2) + abs(y2 - initial_y2)) * lambda;
	}
}